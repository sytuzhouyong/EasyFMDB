...
#define RANGE_SQL(__r) \
    __r.length == 0 ? @"" : [NSString stringWithFormat:@"limit %ld offset %ld", __r.length, __r.location]
    ...
    @synchronized(self)
    {
        ...
        if (_dbQueue == nil || _dbQueue.path.length == 0)
        {
            ...
            else
            {
                ...
                if (![db open])
                {
                    NSLog(@"oh no, open db in (%@) failed!", dbPath);
                    exit(0);
                }
...
+ (void)createTablesInDBPath:(NSString *)dbPath
{
    do
    {
        ...
        for (NSValue *value in registedModels)
        {
            ...
            {
                NSLog(@"oh no, create table(%@) failed!", TABLE_NAME_C(clazz));
                break;
            }
        }
    ...
    NSLog(@"oh no, create table failed!");
    exit(0);
}
...
+ (void)updateTableInDB:(FMDatabase *)db
{
    ...
    for (NSValue *value in registedModels)
    {
        ...
        if (![db tableExists:tableName])
        {
            ...
            {
                NSLog(@"oh no, create table (%@) failed!", tableName);
                exit(0);
            }
        }
        else
        {
            ...
            {
                NSLog(@"db begin transaction failed!");
                exit(0);
            }
...
- (BOOL)addModel:(ZyxBaseModel *)model inDatabase:(FMDatabase *)db
{
    ...
    if (!result)
    {
        NSLog(@"db add model %@ failed, error code: %d, erro message: %@", model, db.lastErrorCode, db.lastErrorMessage);
    }
    else
    {
        long long last = [db lastInsertRowId];
        NSLog(@"last insert row id = %lld, in table : %@", last, TABLE_NAME(model));
        model.id = last;
    }
    return result;
}
             ...
             result:(DBUpdateOperationResultBlock)block
{
    if (updates.count == 0)
    {
        NSLog(@"oh no, update with no property");
        EXECUTE_BLOCK1(NO);
    }
...
- (BOOL)update:(id)param result:(DBUpdateOperationResultBlock)block
{
    ...
    else if ([param isKindOfClass:NSDictionary.class])
    {
        ...
        if (model == nil)
        {
            NSLog(@"oh no, kEasyFMDBModel in dict(%@) is nil!", dict);
            return NO;
        }
        ...
        if (u == nil)
        {
            ...
            {
                NSLog(@"oh no, nil kEasyFMDBQueryProperties and nil kEasyFMDBQueryProperties* in dict(%@), "
        ...
        else
        {
            ...
            if (updates == nil)
            {
                NSLog(@"oh no, invalid param of kEasyFMDBUpdateProperties in dict(%@)", dict);
                return NO;
            }
...
- (void)query:(id)param result:(DBQueryOperationResultBlock)block
{
    ...
    else if ([param isKindOfClass:NSDictionary.class])
    {
        ...
        {
            NSLog(@"parse query param(%@) failed!", param);
            return;
        }
        ...
        while ([result next])
        {
            ...
            for (NSString *name in propertyDict.allKeys)
            {
                ...
                switch (p.type)
                {
                    ...
                    default:
                    {
                        NSLog(@"unrecognized data type(%lu)!", p.type);
                        break;
                    }
                }
            }
...
- (NSArray *)makeQueryPredicateSql:(ZyxBaseModel *)model withParam:(NSDictionary *)dict
{
    ...
    for (NSUInteger i=0; i<valuesCount; i++)
    {
        ...
        {
            NSLog(@"property at index %lu is empty", i);
            continue;
        }
        ...
        if (p == nil)
        {
            NSLog(@"update unknown property(%@) in class(%@)", property, NSStringFromClass(model.class));
            continue;
        }
        ...
        {
            NSLog(@"property values of property %@ at index %lu is empty", property, i);
            continue;
        }
        ...
        for (NSUInteger j=0; j<propertyValuesCount; j++)
        {
            ...
            if (matchType == DCT_Like && [value isKindOfClass:NSString.class]) // %test%
            {
            ...
            [logSQL appendFormat:@"%@ %@ %@ or ", p.nameInDB, match, value];
...
- (NSArray *)makeAssignmentSqlOfModel:(ZyxBaseModel *)model withUpdatedProperties:(NSArray *)updatedPropertes
{
    ...
    for (NSUInteger i=0; i<updateCount; i++)
    {
        ...
        if (property.length == 0)
        {
            NSLog(@"oh no, property %@ at index %lu is nil", updatedPropertes, i);
            continue;
        }
        ...
        if (p == nil)
        {
            NSLog(@"oh no, unknown property(%@) in class(%@)", property, NSStringFromClass(model.class));
            continue;
        }
                    ...
                    logics:(NSArray *)logics
{
    ...
    {
        NSLog(@"makeUpdateSQL param queries(%@) is not a valid object", queries);
        return @[@"", @"", @[]];
    }
...
- (NSString *)makeInsertSQL:(ZyxBaseModel *)model
{
    ...
    for (NSString *key in properties)
    {
        ...
        if (p == nil)
        {
            NSLog(@"makeInsertSQL: unknown property(%@) in class(%@)", key, NSStringFromClass(model.class));
            continue;
        }
...
- (NSString *)makeOrderSQL:(Class)c orders:(NSDictionary *)orders
{
    ...
    for (NSString *key in orderKeys)
    {
        ...
        if (property == nil)
        {
            NSLog(@"orders key(%@) is not exist", key);
            continue;
        }
...
- (void)checkUpdateSQLResult:(FMDatabase *)db
{
    ...
    if (changes < 1)
    {
        NSLog(@"oh no, update sql do not make any changes in db, error code:%d, error message:%@", db.lastErrorCode, db.lastErrorMessage);
    }
    else
    {
        NSLog(@"yes, update sql make effective %d rows", changes);
    }
}
                  ...
                  range:(NSRange *)range
{
    ...
    {
        NSLog(@"parseQueryParam1 : param is not a NSDictionary object!");
        return NO;
    }
    ...
    if (mm != nil)
    {
        ...
        {
            NSLog(@"kEasyFMDBPropertiesValues in dict(%@) is not a valid NSDictionary!", dict);
            return NO;
        }
    }
    ...
    *logics = [self arrayInDictionary:dict forKey:kEasyFMDBLogics];
    ...
    {
        NSLog(@"kEasyFMDBOrders in dict(%@) is invlalid, so set it to nil", dict);
        *orders = nil;
    }
...
- (NSArray *)arrayInDictionary:(NSDictionary *)dict forKey:(NSString *)key
{
    ...
    if (value != nil)
    {
        ...
        {
            NSLog(@"(%@) in dict(%@) is invlalid, so return nil", key, dict);
        }
    }
    return array;
}
